...
simedFPDB4LB_v4.py 
  ...
  simedFPDB4LB_v4: this file is copied from ~_v2.py. 
     this script reads the existing FPDB and tweak it.
     This tweak is polarization angle.

  T. Matsumura, 2014-6-10
'''
...
#    columns = np.array([6,7,8,9,10,11,10,9,8,7,6])
    ...
    # insert the random error in polarization angle. But keep the orthogonality.
    num_all = 0
    num_wafer = 5
    for k in range(0,num_wafer):
        for j in range(0,num_rows):
            for i in range(0,columns[j]):
                num_all += 1
    polang_random_rad = np.random.normal(0.,1.,num_all)*15./radeg
    ...
    for k in range(0,num_wafer):
        ...
        for j in range(0,num_rows):
            ...
            #        print j, i, idx_tmp, columns_i[j], columns_i[j]+az_sep*i, rows[j], polang_index_tmp
                ...
                polang_random_tmp = polang_random_rad[idx_tmp-1]
                ...
                if ((idx_tmp == polang_pixindex[j]) and (polang_index[j] == 1)): 
                    pol_t = -90./radeg+polang_random_tmp
                    pol_b = 0./radeg+polang_random_tmp
                    polang.append(pol_t)
                    polang.append(pol_b)
                ...
                if ((idx_tmp == polang_pixindex[j]) and (polang_index[j] == -1)):
                    pol_t = -135./radeg+polang_random_tmp
                    pol_b = -45./radeg+polang_random_tmp
                    polang.append(pol_t)
                    polang.append(pol_b)
                    polang_t.append(pol_t)
                    polang_b.append(pol_b)
