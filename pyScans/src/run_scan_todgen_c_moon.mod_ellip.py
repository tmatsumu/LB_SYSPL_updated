...
if option_sh=='y':
    ...
    theta_antisun_long = np.float(sys.argv[5])/radeg
    theta_antisun_short = np.float(sys.argv[6])/radeg
    freq_antisun = 1./(np.float(sys.argv[7])*60.)
    theta_boresight = np.float(sys.argv[8])/radeg
    freq_boresight = np.float(sys.argv[9])/60.
    ydays = np.int(sys.argv[10])
    nside = np.int(sys.argv[11])
...
print '  theta_antisun_l:', theta_antisun_long*radeg, type(theta_antisun_long)
print '  theta_antisun_s:', theta_antisun_short*radeg, type(theta_antisun_short)
...
filename = title+'_SCANSPEC_'+str(int(total_time/60.))+'min_'+str(theta_antisun_long*radeg)+'_'+str(theta_antisun_short*radeg)+'degs_'+str(1./freq_antisun/60.)+'min_'+str(theta_boresight*radeg)+'degs_'+str(freq_boresight*60.)+'rpm_'+str(int(ydays))+'day_nside'+str(int(nside))+'_'+str(int(sample_rate))+'Hz'
...
for i in range(0,ydays):
    ...
    mapout = liblb.gen_scan_c_mod_moon_ellip(theta_antisun_long, theta_antisun_short, theta_boresight, \
                                                 freq_antisun, freq_boresight, total_time, today_julian, \
                                                 sample_rate, dir_out, filename, title, nside, runtime_i, option_gen_ptg)
